        ...
        if not options.with_aggregation:
            ...
            with tf.variable_scope('aggregation_layer'):
                for i in xrange(options.aggregation_layer_num):
                    with tf.variable_scope('layer-{}'.format(i)):
                        aggregation_lstm_cell_fw = tf.contrib.rnn.BasicLSTMCell(options.aggregation_lstm_dim)
                        aggregation_lstm_cell_bw = tf.contrib.rnn.BasicLSTMCell(options.aggregation_lstm_dim)
                        if is_training:
