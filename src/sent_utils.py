...
class QASentence(object):
    def __init__(self, rawText, annotation, ID_num=None, isLower=False, end_sym=None, is_question=False): ###
        self.rawText = rawText
        self.annotation = annotation
        self.tokText = annotation['toks']
        # it's the answer sequence
        if end_sym != None:
#             self.rawText = self.rawText.replace('<s>','<b>',10)
#             self.rawText = self.rawText.replace('</s>','</b>',10)
#             self.tokText = self.tokText.replace('<s>','<b>',10)
#             self.tokText = self.tokText.replace('</s>','</b>',10)
        ...
        if is_question==True :   ###
            ...
            pos_tags = self.POSs.strip().split(" ")
            negs = self.NERs.strip().split(" ")
            template = []
            for i in range(len(pos_tags)):
                if tokens[i] in interrogative_words:
    ...
    def convert2index(self, word_vocab, char_vocab, POS_vocab, NER_vocab, template_vocab, max_char_per_word=-1):
        ...
        if template_vocab is not None:
            self.template_idx_seq = template_vocab.to_index_sequence(self.question_template) ###
