...
class CovCopyAttenGen:
    ...
    def gen_question_template(self, inputs, lens=None):
        ...
        lens : [batch_size]
        '''
        embeddings = self.embedding_lookup(inputs, True)
        # embedding_shape = tf.shape(embeddings)[-1]
        if lens != None:
            mask = tf.sequence_mask(lens, self.options.max_answer_len, dtype=tf.float32)
            mask = tf.expand_dims(mask, axis=-1)

            embeddings = embeddings * mask

            # average question embeddings as question template
            question_template = tf.reduce_sum(embeddings, axis=1)/tf.cast(tf.expand_dims(lens, axis=-1), dtype=tf.float32)
        else:
    ...
    def decode_mode(self, word_vocab, question_template, template_len, beam_size, state_t_1, context_t_1, coverage_t_1, word_t,
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            word_t_representation = self.embedding_lookup(word_t, False)
...
def CE_loss(word_probs, answers, loss_weights):
    ...
    loss_weights: [batch_size, max_dec_steps]
    '''
