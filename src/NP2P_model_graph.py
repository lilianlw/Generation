...
ROUGE_path = 'src/pythonrouge/RELEASE-1.5.5/ROUGE-1.5.5.pl'
data_path = 'src/pythonrouge/RELEASE-1.5.5/data'
...
class ModelGraph(object):
    def __init__(self, template_vocab=None, word_vocab=None, char_vocab=None, POS_vocab=None, NER_vocab=None, options=None, mode='ce_train'): ###
        ...
        if options.two_sent_inputs: # take two sentences as inputs
            self.encoder = matching_encoder_utils.MatchingEncoder(self, options,
                                word_vocab=word_vocab, char_vocab=char_vocab, POS_vocab=POS_vocab, NER_vocab=NER_vocab)
        ...
        with variable_scope.variable_scope("generator"):
            ...
            if mode == 'decode':
                ...
                _, _, self.greedy_words = self.generator.train_mode(word_vocab, self.encode_dim, self.phrase_representations, self.encoder_features,
                ...
                _, self.loss, _ = self.generator.train_mode(word_vocab, self.encode_dim, self.phrase_representations,self.encoder_features,
                        self.phrase_idx, self.phrase_mask, self.init_decoder_state,
                        self.gen_input_words, self.in_answer_words, loss_weights, mode_gen='loss')

                tf.get_variable_scope().reuse_variables()

                _, _, self.sampled_words = self.generator.train_mode(word_vocab, self.encode_dim, self.phrase_representations,self.encoder_features,
                        self.phrase_idx, self.phrase_mask, self.init_decoder_state,
    ...
    def run_encoder(self, sess, batch, options, only_feed_dict=False):
        ...
        if options.with_NER: feed_dict[self.in_passage_NERs] = batch.sent1_NER

        if options.two_sent_inputs:
            feed_dict[self.question_lengths] = batch.sent3_length   ###answer
            if options.with_word: feed_dict[self.in_question_words] = batch.sent3_word
            if options.with_char:
                feed_dict[self.question_char_lengths] = batch.sent3_char_lengths
                feed_dict[self.in_question_chars] = batch.sent3_chars
