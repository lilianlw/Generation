...
def read_all_GenerationDatasets(inpath, isLower=True):
    ...
    for instance in dataset:
        ...
        sent1 = QASentence(text1, annotation1, ID_num=ID_num, isLower=isLower, is_question=False) ###

        text2 = instance['text2']
        if text2 == "": continue
        annotation2 = instance['annotation2']
        sent2 = QASentence(text2, annotation2, ID_num=ID_num, isLower=isLower, end_sym='</s>', is_question=True) ###
        ...
        if instance.has_key('text3'):
            ...
            sent3 = QASentence(text3, annotation3, ID_num=ID_num, isLower=isLower, is_question=False) ###
...
def collect_vocabs(all_instances):
    ...
    for (sent1, sent2, sent3) in all_instances:
        ...
        all_template_words.update(re.split("\\s+", sent2.question_template)) ###
    all_chars = set()
    for word in all_words:
        for char in word:
            all_chars.add(char)
    return (all_words, all_chars, all_POSs, all_NERs, all_template_words) ###

class QADataStream(object):
    def __init__(self, all_questions, word_vocab=None, char_vocab=None, POS_vocab=None, NER_vocab=None, template_vocab=None, options=None, ###
        ...
        for (sent1, sent2, sent3) in all_questions:# sent1 is the long passage or article
            ...
            sent1.convert2index(word_vocab, char_vocab, POS_vocab, NER_vocab, None, max_char_per_word=options.max_char_per_word) ###
            sent2.convert2index(word_vocab, char_vocab, POS_vocab, NER_vocab, template_vocab, max_char_per_word=options.max_char_per_word) ###
            if sent3 is not None:
                sent3.convert2index(word_vocab, char_vocab, POS_vocab, NER_vocab, None, max_char_per_word=options.max_char_per_word) ###
        ...
        self.num_batch = len(self.batches) ##1510
