...
def main(_):
    ...
    path_prefix = log_dir + "/NP2P.{}".format(FLAGS.suffix) ##logs/NP2P.mpqg_1
    ...
    print('Number of training samples: {}'.format(len(trainset))) ##75500
    ...
    best_path = path_prefix + ".best.model" ##'logs/NP2P.mpqg_1.best.model'
    if os.path.exists(best_path + ".index"):
        ...
        print('Number of words: {}'.format(len(allWords))) ##70086
        print('Number of allChars: {}'.format(len(allChars))) ##906
        print('Number of allPOSs: {}'.format(len(allPOSs))) ##45
        print('Number of allNERs: {}'.format(len(allNERs))) ##12
    ...
    print('Number of batches in trainDataStream: {}'.format(trainDataStream.get_num_batch())) ##1510
    print('Number of batches in devDataStream: {}'.format(devDataStream.get_num_batch())) ##359
    sys.stdout.flush()
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Train"):
            with tf.variable_scope("Model", reuse=None, initializer=initializer):
                train_graph = ModelGraph(template_vocab=template_vocab, word_vocab=word_vocab, char_vocab=char_vocab, POS_vocab=POS_vocab,
